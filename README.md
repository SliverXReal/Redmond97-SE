...
It is a fork of the Redmond97 project, but does not focus on recreating a clone of the Windows 95 desktop as a whole.
...
4) Add the following to /etc/environment to make it global, or to something that runs when you login for your account:

   QT_QPA_PLATFORMTHEME=qt5ct

   QT_SCALE_FACTOR=1

5) Set font size to 18 from 9 in qt5ct and qt6 ct.

6) In Xfce4, override xfsettingsd's meddling to fix GTK2 icon sizes either by:
   ...
   This will make GTK2/3/4 and Qt5/6 applications mostly look right. Icons will be small under Qt apps, however.
...
Xfce4 overrides the theme's button order definitions for these.
...
Icons are small due to Xfce4's 2x Scale function not working for GTK2, requiring double scaled assets/RC files to compensate.
Doubling Qt's scaling would make everything in them 200% bigger. I may look into creating a companion set of Qt5/Qt6 themes
to the GTK ones at some point in the future.
...
that amount, not just text.. It appears to be a conflict with using GDK_SCALE or Xfce4's Windows Scale 2x function.
